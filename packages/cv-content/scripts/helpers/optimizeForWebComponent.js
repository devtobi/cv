...
const replaceInCSSofHTML = async (inputPath, outputPath, searchString, replacementString) => {
	console.log(`Replacing ${searchString} with ${replacementString} in ${inputPath}`);
	...
	const result = data.replace(cssRegex, (match, cssContent) => {
		const modifiedStyleContent = cssContent.replace(replaceRegex, replacementString);
